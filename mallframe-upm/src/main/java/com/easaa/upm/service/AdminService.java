...
public class AdminService extends EABaseService{
	...
	public static final String  SESSION_CHECKCODE ="loginCheckCode";
	...
	public void doLogin(PageData pd) throws Exception{
			...
			String checkCode = KEYDATA[2];
			if (checkCode.isEmpty()){
				throw new Exception("登录验证码不能为空");
			}
			if (!checkCode.equalsIgnoreCase((String)session.getAttribute(SESSION_CHECKCODE))){
				throw new Exception("checkCodeError");
			}
...
//			String passwd = new SimpleHash("SHA-1", USERNAME, PASSWORD).toString();	//密码加密
			...
			if(pd != null){
				...
				pd.put("IP",EAWEB.getIp().toString());
