...
import com.easaa.goods.dao.MallCategoryMapper;

import net.sf.json.JSONArray;

@Service
public class MallCategoryService extends EABaseService {
	...
	public PageData selectOrder(PageData pageData) {
		 ...
		 * 查询最大值
		 */
		List<PageData> result = mallCategoryMapper.selectMaxOrder(pageData);
	...
	public String  categoryEncapsulationToJson(PageData pd){
		Page page = new Page();
		page.setPd(pd);
		List<PageData> varList = mallCategoryMapper.selectByMap(page);
	...
	public List<PageData> selectHeatList() {
		return mallCategoryMapper.selectHeatList();
	}
}
