...
public class EAXml {
	private static Log log = LogFactory.getLog(EAXml.class);
	...
	public static final PageData parseXml2PageDataBasedNode(String pStrXml) {
		PageData outDto = new PageData();
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static final PageData parseXml2PageDataBasedNode(String pStrXml, String pXPath) {
		PageData outDto = new PageData();
		String strTitle = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>";
		Document document = null;
		try {
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static final PageData parseXml2PageDataBasedProperty(String pStrXml, String pXPath) {
		PageData outDto = new PageData();
		String strTitle = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>";
		Document document = null;
		try {
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static final String parsePageData2Xml(PageData pd, String pRootNodeName) {
		if (EAUtil.isEmpty(pd)) {
		...
		Iterator keyIterator = pd.keySet().iterator();
		while (keyIterator.hasNext()) {
			String key = (String) keyIterator.next();
			String value = pd.getAsString(key);
	 ...
	 */
	@SuppressWarnings("rawtypes")
	public static final String parsePageData2Xml(PageData pd, String pRootNodeName, String pFirstNodeName) {
		if (EAUtil.isEmpty(pd)) {
			log.warn("传入的DTO对象为空,请确认");
			return "<root />";
		}
