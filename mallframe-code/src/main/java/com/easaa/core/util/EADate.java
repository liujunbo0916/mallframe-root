...
public class EADate {
	...
	public static Timestamp getCurrentTimestamp() {
		Object obj = null;
		try {
			obj = EATypeCase.convert(getCurrentTime(), "Timestamp", "yyyy-MM-dd HH:mm:ss");
		} catch (Exception e) {
			e.printStackTrace();
		}
		if (obj != null)
	 ...
	 * @return
	 */
	public static String prettyTime(String time) {
		try {
			if (EAUtil.isNotEmpty(time)) {
				SimpleDateFormat sdf = new SimpleDateFormat(FORMAT_LONG);
				PrettyTime ptime = new PrettyTime();
				return (ptime.format(sdf.parse(time))).replace(" ", "");
			} else {
				return "";
			}
		} catch (Exception e) {
